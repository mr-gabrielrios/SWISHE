  ...
  do nc = 1, num_cpld_calls
     ...
     if( Atm%pe )then
        ...
        do na = 1, num_atmos_calls
           ...
           if(do_chksum) then
                   call atmos_ice_land_chksum('update_atmos_down+', (nc-1)*num_atmos_calls+na)
           endif
           ...
           if(do_chksum) then
                   call atmos_ice_land_chksum('flux_down_from_atmos+', (nc-1)*num_atmos_calls+na)
           endif
           ...
           if(do_chksum) then
               write(*,*) "writing checksum"
               call atmos_ice_land_chksum('update_atmos_up+', (nc-1)*num_atmos_calls+na)
           endif
